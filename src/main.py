        ...
        for predict_horizon in self.params.prediction_horizon_list:
            ...
            for segment in self.params.segments_list:
                ...
                for model in models["ph_models"]:
                    ...
                    result['mean_test_score'] = model.cv_results['mean_test_score'][model.best_index]
                    scores.append(result['mean_test_score'])
                    for i in range(self.params.n_folds):
                        result['split'+str(i)+'_test_score'] = model.cv_results['split'+str(i)+'_test_score'][model.best_index]
                ...
                self.log.info("Save model for segment {} prediction horizon {}".format(segment, str(predict_horizon)))
